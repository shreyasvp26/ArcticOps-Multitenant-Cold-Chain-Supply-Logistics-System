...
import { Bell, AlertTriangle, CheckCircle2, Info, Clock, X } from "lucide-react"
...
const SEVERITY_CONFIG = {
  ...
  emergency: { color: "#FF4757", icon: AlertTriangle, label: "Emergency", bg: "rgba(255,71,87,0.1)" },
...
export default function ClientNotificationsPage() {
  ...
  // Only show notifications relevant to this client's tenant
  const myNotifs = notifications.filter(
    (n) => n.tenantId === user?.tenantId || n.tenantId === null
  )
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ background: "linear-gradient(135deg, rgba(59,130,246,0.2) 0%, rgba(59,130,246,0.08) 100%)", border: "1px solid rgba(59,130,246,0.25)" }}
          >
          ...
          return (
            <button
              ...
              style={{
                ...
                border: isActive ? "1px solid rgba(59,130,246,0.3)" : "1px solid var(--ao-border)",
                fontFamily: "var(--ao-font-body)",
              }}
            >
              {label}
      ...
      >
        {filtered.length === 0 ? (
          ...
          filtered.map((n) => {
            ...
            const Icon = cfg.icon
            const timeAgo = (() => {
              try { return formatDistanceToNow(parseISO(n.createdAt), { addSuffix: true }) }
            ...
            return (
              <div
                key={n.id}
                className={cn(
                  "group relative flex items-start gap-3 px-4 py-4 border-b last:border-b-0 transition-colors cursor-pointer",
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-[13px] font-semibold leading-tight"
                      style={{ color: n.read ? "var(--ao-text-secondary)" : "var(--ao-text-primary)", fontFamily: "var(--ao-font-body)" }}>
                      {n.title}
                    </p>
                    {!n.read && <span className="w-2 h-2 rounded-full shrink-0 mt-1" style={{ background: cfg.color }} />}
                  </div>
