    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 relative" style={{ backgroundColor: "#0A1628" }}>
        <svg viewBox={`0 0 100 60`} className="w-full h-full" preserveAspectRatio="xMidYMid slice">
          ...
          {/* Completed route */}
          <line x1={ox} y1={oy} x2={cx} y2={cy} stroke="#00D4AA" strokeWidth="0.8" />
          {/* Remaining route */}
          <line x1={cx} y1={cy} x2={dx} y2={dy} stroke="#243352" strokeWidth="0.6" strokeDasharray="2,1.5" />
          {/* Origin/destination */}
          ...
          {checkpoints.map((cp) => {
            ...
            return <circle key={cp.id} cx={cpx} cy={cpy} r="1.2" fill={cp.status === "passed" ? "#2ED573" : cp.status === "current" ? "#00D4AA" : "#374151"} />
          })}
          {/* Current position with pulse */}
          <circle cx={cx} cy={cy} r="3" fill="#00D4AA" fillOpacity="0.1">
            <animate attributeName="r" from="2" to="5" dur="2s" repeatCount="indefinite" />
            <animate attributeName="opacity" from="0.3" to="0" dur="2s" repeatCount="indefinite" />
          </circle>
          <circle cx={cx} cy={cy} r="2" fill="#00D4AA" />
        </svg>

        {/* Next checkpoint overlay */}
        {nextCheckpoint && (
          <div className="absolute bottom-4 left-4 right-4 rounded-xl p-3"
            style={{ backgroundColor: "rgba(17,29,51,0.95)", backdropFilter: "blur(12px)", border: "1px solid var(--ao-border)" }}>
      ...
      {/* Checkpoint list */}
      <div className="border-t p-3 max-h-48 overflow-y-auto" style={{ borderColor: "var(--ao-border)", backgroundColor: "rgba(12,22,42,0.9)" }}>
        {checkpoints.map((cp) => (
          <div key={cp.id} className="flex items-center gap-3 py-2 border-b last:border-b-0" style={{ borderColor: "var(--ao-border)" }}>
            <div className={`w-2 h-2 rounded-full shrink-0`}
              style={{ backgroundColor: cp.status === "passed" ? "#2ED573" : cp.status === "current" ? "#00D4AA" : "#374151" }} />
